...
from tensorflow.keras.layers import Input, Conv1D, MaxPooling1D, UpSampling1D, Dense, Reshape, Flatten
...
# Reshape for 1D CNN (32 time steps, 1 channel)
X = X.reshape((X.shape[0], X.shape[1], 1))  # Shape: (35, 32, 1)
...
input_dim = X_train.shape[1]  # 32 (time steps)
encoding_dim = 8  # Reduced encoding dimension for 1D CNN

input_layer = Input(shape=(input_dim, 1))  # (32, 1)

# Encoder
encoded = Conv1D(8, kernel_size=3, activation='relu', padding='same', kernel_regularizer=l2(0.01))(input_layer)
encoded = MaxPooling1D(pool_size=2)(encoded)  # Reduce to 16 time steps
encoded = Conv1D(4, kernel_size=3, activation='relu', padding='same', kernel_regularizer=l2(0.01))(encoded)
encoded = MaxPooling1D(pool_size=2)(encoded)  # Reduce to 8 time steps

# Bottleneck (encoded representation)
encoded = Flatten()(encoded)  # Flatten to 8 * 1 = 8 features
encoded = Dense(encoding_dim, activation='relu', kernel_regularizer=l2(0.01))(encoded)

# Decoder
decoded = Dense(8, activation='relu', kernel_regularizer=l2(0.01))(encoded)  # Match bottleneck size
decoded = Reshape((8, 1))(decoded)  # Reshape back to (8, 1)
decoded = UpSampling1D(size=2)(decoded)  # Increase to 16 time steps
decoded = Conv1D(4, kernel_size=3, activation='relu', padding='same', kernel_regularizer=l2(0.01))(decoded)
decoded = UpSampling1D(size=2)(decoded)  # Increase to 32 time steps
decoded = Conv1D(1, kernel_size=3, activation='sigmoid', padding='same')(decoded)  # Output 32 time steps, 1 channel
...
plt.title('Convolutional Autoencoder Training and Validation Loss for c6288', fontsize=14)
...
encoder = Model(inputs=autoencoder.input, outputs=autoencoder.layers[4].output)  # Before bottleneck (Dense layer)
X_train_encoded = encoder.predict(X_train)  # Predict first, then reshape
X_train_encoded = X_train_encoded.reshape(X_train_encoded.shape[0], -1)  # Reshape to 2D
X_test_encoded = encoder.predict(X_test)  # Predict first, then reshape
X_test_encoded = X_test_encoded.reshape(X_test_encoded.shape[0], -1)  # Reshape to 2D

# Fault detection
reconstructed_train = autoencoder.predict(X_train)
reconstructed_train = reconstructed_train.reshape(reconstructed_train.shape[0], reconstructed_train.shape[1])  # Flatten last dim
mse_train = np.mean(np.power(X_train.reshape(X_train.shape[0], X_train.shape[1]) - reconstructed_train, 2), axis=1)
...
reconstructed_test = reconstructed_test.reshape(reconstructed_test.shape[0], reconstructed_test.shape[1])  # Flatten last dim
mse_test = np.mean(np.power(X_test.reshape(X_test.shape[0], X_test.shape[1]) - reconstructed_test, 2), axis=1)
...
reconstructed_all_binary = (reconstructed_all.reshape(reconstructed_all.shape[0], reconstructed_all.shape[1]) > 0.5).astype(int)
