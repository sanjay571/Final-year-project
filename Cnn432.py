...
# Ensure all patterns are 36 bits
...
# Convert to NumPy array and reshape for 1D convolution (36 timesteps, 1 channel)
X = np.array([list(map(int, list(seq))) for seq in test_patterns_fixed]).astype(np.float32)
X = X.reshape((X.shape[0], X.shape[1], 1))  # Shape: (63, 36, 1)
...
input_dim = X_train.shape[1]  # 36 timesteps
...
encoded = MaxPooling1D(pool_size=2)(encoded)  # Reduces to (18, 32)
encoded = Conv1D(16, kernel_size=3, activation='relu', padding='same')(encoded)
encoded = MaxPooling1D(pool_size=2)(encoded)  # Reduces to (9, 16)
encoded = Dropout(0.2)(encoded)

# Decoder
decoded = Conv1D(16, kernel_size=3, activation='relu', padding='same')(encoded)
decoded = UpSampling1D(size=2)(decoded)  # Upsamples to (18, 16)
decoded = Conv1D(32, kernel_size=3, activation='relu', padding='same')(decoded)
decoded = UpSampling1D(size=2)(decoded)  # Upsamples to (36, 32)
decoded = Dropout(0.2)(decoded)
decoded = Conv1D(channels, kernel_size=3, activation='sigmoid', padding='same')(decoded)  # Output shape: (36, 1)
...
# Flatten the encoded features (from (samples, 9, 16) to (samples, 9*16))
...
for i in range(len(X)):
    original = ''.join(map(str, X[i].flatten().astype(int)))  # Flatten to 1D and convert to string
    print(f"Pattern {i+1}: {original}")

print("\nReconstructed 63 Test Patterns:")
for i in range(len(X)):
    reconstructed = ''.join(map(str, reconstructed_all_binary[i].flatten().astype(int)))  # Flatten to 1D and convert to string
