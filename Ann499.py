...
# Synthetic labels (fixed with seed for reproducibility; replace with real labels if available)
...
# ✅ Step 3: Build Improved Autoencoder Model
input_dim = X_train.shape[1]  # 73 bits
encoding_dim = 32

input_layer = Input(shape=(input_dim,))
encoded1 = Dense(128, activation='relu')(input_layer)  # Increased capacity
encoded1 = Dropout(0.2)(encoded1)  # Regularization
...
autoencoder.compile(optimizer=Adam(learning_rate=0.00005), loss='binary_crossentropy')  # Lower learning rate
...
encoder = Model(inputs=autoencoder.input, outputs=autoencoder.layers[4].output)  # Layer 4 is encoded3
...
threshold = np.mean(mse_train) + 1.5 * np.std(mse_train)  # Adjusted threshold
...
# ✅ Step 7: Train Random Forest with Adjusted Parameters
